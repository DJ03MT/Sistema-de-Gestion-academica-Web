...
// Campos de la ficha (Tu array original)
const camposFicha = [
...
// =================================================================
// RENDER: Formulario para AGREGAR estudiante (Sin cambios)
// =================================================================
...
// =================================================================
// CREATE: Procesar NUEVO estudiante (Corregido para no enviar FechaCreacion)
// =================================================================
export const crearEstudiante = async (req, res) => {
    ...
    try {
        ...
        // --- PASO 3: Crear la Ficha Estudiantil ---
        // (Quitamos FechaCreacion de los arrays cols/vals)
        ...
        // --- PASO 4: Vincular la Ficha al Estudiante ---
...
// =================================================================
// RENDER: Formulario para EDITAR estudiante (ACTUALIZADO)
// =================================================================
export const renderEditarEstudianteForm = async (req, res) => {
    try {
        ...
        // --- AÑADIDO: Cargar la lista de usuarios (solo roles Estudiante o Pendiente) ---
        ...
        res.render('secretaria/AdminEstudiantes/editar-estudiante', {
            ...
            grados: gradosResult.recordset,
            secciones: seccionesResult.recordset,
            usuarios: usuariosResult.recordset, // --- AÑADIDO: Pasar usuarios a la vista
            error: req.query.error || null,
...
// =================================================================
// UPDATE: Procesar EDICIÓN estudiante (Actualizado)
// =================================================================
export const actualizarEstudiante = async (req, res) => {
    ...
    const { 
        ...
        ID_Usuario, // --- AÑADIDO: Recibir el ID_Usuario del formulario
        ...fichaData
    } = req.body;
    ...
    try {
        ...
        // --- PASO 1: Obtener IDs de la NUEVA data ---
        const idsResult = await transaction.request()
            .input('NombreGrado', sql.VarChar, grado)
            .input('NombreSeccion', sql.VarChar, seccion)
            .query(`
        ...
        // --- PASO 2: Obtener IDs de la VIEJA data ---
        ...
        // --- PASO 3: Actualizar Estudiante (Datos Biográficos Y ID_Usuario) ---
        // --- AÑADIDO: Manejo de ID_Usuario nulo ---
        ...
        // --- PASO 5: VERIFICAR SI CAMBIÓ DE GRADO O SECCIÓN ---
        if (ID_Grado_Viejo !== ID_Grado_Nuevo || ID_Seccion_Vieja !== ID_Seccion_Nueva) {
            ...
            // 5.2. Encontrar TODOS los cursos para el NUEVO Grado/Sección/Año
            ...
            // 5.3. Inscribir al estudiante en CADA NUEVO curso
...
// =================================================================
// DELETE: Retirar estudiante (Sin cambios)
// =================================================================
export const retirarEstudiante = async (req, res) => {
    ...
    try {
        ...
        // 1. Poner estado "Retirado" al estudiante y guardar el motivo/fecha
        ...
        // 2. BORRAR todas las inscripciones del año activo
        ...
        // 3. (Opcional) Actualizar Ficha Activa con una nota de retiro
...
// =================================================================
// RENDER: Formulario de MATRÍCULA (Estudiante) (Sin cambios)
// =================================================================
...
// =================================================================
// CREATE: Procesar MATRÍCULA (Estudiante) (Sin cambios)
// =================================================================
export const procesarFormularioMatricula = async (req, res) => {
    ...
    try {
        ...
        // --- PASO 3: Crear la nueva Ficha Estudiantil (y marcarla como ACTIVA) ---
        const fichaRequest = transaction.request();
        ...
        fichaRequest.input('EstadoFicha', sql.VarChar, 'Activa'); // <-- CAMBIO: Se activa de inmediato
        ...
        // --- PASO 4: Vincular la nueva Ficha al Estudiante ---
        await transaction.request()
        ...
        // --- PASO 5: Inscribir automáticamente en los cursos ---
        const cursosNuevos = await transaction.request()
...
// =================================================================
// GENERATE PDF: (Sin cambios)
// =================================================================
// ... (Las funciones generarPDFDesdeHTML, generarMatriculaPDF, etc. no se modifican) ...
...
// =================================================================
// --- INICIO DE LA NUEVA FUNCIÓN: GENERAR BOLETÍN PDF ---
// =================================================================
...
export const generarBoletinPDF = async (req, res) => {
    try {
        ...
        // 1. Obtener ID del Estudiante y Año Activo
        ...
        // 2. Obtener toda la información del estudiante y sus notas
        const infoQuery = await pool.request()
            ...
            .query(`
                ...
                -- Consulta 3: Conteo de Asistencias (Ejemplo básico, requiere ajustar)
                SELECT 
